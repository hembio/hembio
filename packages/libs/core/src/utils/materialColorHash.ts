export function getColorFromString(
  ...
  if (!s || typeof s !== "string") {
    throw Error("Error: String not valid");
  }
  const maxColor = darkColors ? 18 : 15;
  let maxTone = accentColors ? 13 : 9;

  const color = djb2(s) % maxColor;
  ...
  const tone = djb2(s) % maxTone;
  return DEFAULT_PALETTE[tone][color];
}

export function getColorFromNumber(
  x: string,
  darkColors: boolean,
